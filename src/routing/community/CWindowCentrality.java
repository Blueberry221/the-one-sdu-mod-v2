 ...
 * 
 * <p>
 * CWindow estimates the node's centrality using the node's average degree
...
public class CWindowCentrality implements Centrality {
	...
	public double getGlobalCentrality(Map<DTNHost, List<Duration>> connHistory) {
		...
		for (Map.Entry<DTNHost, List<Duration>> entry : connHistory.entrySet()) {
			...
			for (Duration d : entry.getValue()) {
				...
				// System.out.println("nodesAlreadyCounted.contains(h):
				// "+nodesAlreadyCounted.contains(h));
				// else
				...
				// System.out.println("centralities["+epoch+"]: " + centralities[epoch] + ",
				// size: " + centralities.length);
				// System.out.println("epoch: "+epoch);
				centralities[epoch]++;
		...
		this.globalCentrality = ((double) sum) / control; // CHANGED FROM this.globalCentrality = ((double)sum) /
															// EPOCH_COUNT;
	...
	@Override
	@SuppressWarnings("rawtypes") // Tambahkan ini untuk menghilangkan warning
	public int[] getGlobalArrayCentrality(java.util.Map connHistory) {
		return new int[0];
	}
