...
public class IntercontactPeopleRankEngine implements RoutingDecisionEngine {
    ...
    private Map<DTNHost, Double> lastContactTime = new HashMap<>();
    private Map<DTNHost, Double> averageICT = new HashMap<>();

    /**
     * Konstruktor utama
     */
    public IntercontactPeopleRankEngine(Settings s) {
        ...
        System.out.println("DEBUG: Damping Factor Terbaca = " + this.dampingFactor);
    }

    /**
     * Copy Constructor
     */
    private IntercontactPeopleRankEngine(IntercontactPeopleRankEngine prototype) {
        this.myRank = prototype.myRank;
        this.dampingFactor = prototype.dampingFactor;
        this.friends = new HashSet<>();
        this.lastContactTime = new HashMap<>();
        this.averageICT = new HashMap<>();
    }

    // while i is in contact with j d
    @Override
    public void doExchangeForNewConnection(Connection con, DTNHost peer) {
        IntercontactPeopleRankEngine peerEngine = getOtherEngine(peer);
    ...
    public boolean shouldSendMessageToHost(Message m, DTNHost otherHost, DTNHost thisHost) {
        ...
        IntercontactPeopleRankEngine otherEngine = getOtherEngine(otherHost);
        if (otherEngine == null)
            return false;

        DTNHost dest = m.getTo();

        // 1. Logika Paper: Cek Popularitas
        boolean betterRank = otherEngine.getRank() >= this.myRank;

        // --- PERBAIKAN ICT ---
        // Gunakan method getter untuk mengambil data dari engine tetangga
        double myICTToDest = this.getAverageICT(dest);
        double otherICTToDest = otherEngine.getAverageICT(dest);

        boolean fasterToDest = otherICTToDest < myICTToDest;
        // ----------------------

        return betterRank || fasterToDest;
    }
    ...
    public double getAverageICT(DTNHost target) {
        // Jika belum pernah bertemu, kembalikan nilai maksimal (penalti)
        return this.averageICT.getOrDefault(target, Double.MAX_VALUE);
    }

    @Override
    public RoutingDecisionEngine replicate() {
        // Membuat salinan engine untuk node-node lain di simulator
        return new IntercontactPeopleRankEngine(this);
    }

    private IntercontactPeopleRankEngine getOtherEngine(DTNHost h) {
        MessageRouter otherRouter = h.getRouter();
        if (otherRouter instanceof DecisionEngineRouter) {
            DecisionEngineRouter deRouter = (DecisionEngineRouter) otherRouter;
            RoutingDecisionEngine engine = deRouter.getDecisionEngine();
            if (engine instanceof IntercontactPeopleRankEngine) {
                return (IntercontactPeopleRankEngine) engine;
            }
        }
        return null;
    }
    ...
    public void connectionUp(DTNHost thisHost, DTNHost peer) {
        // Logika Paper: Tambah teman unik
        friends.add(peer);

        // --- TAMBAHAN ICT ---
        double currentTime = SimClock.getTime();
        if (lastContactTime.containsKey(peer)) {
            double currentICT = currentTime - lastContactTime.get(peer);
            double oldAvg = averageICT.getOrDefault(peer, currentICT);

            // Menggunakan rata-rata sederhana (atau EMA jika mau lebih canggih)
            averageICT.put(peer, (oldAvg + currentICT) / 2);
        }
        lastContactTime.put(peer, currentTime);
        // --------------------
    }
